...
run_machine_learning <- function(x, ..., y = NULL, p = NULL, subset = NULL,
                                 model='rf') {
    y <- unlist(y)
    ...
    if (class(output$predictions_train)=='factor') {
      output$predictions <- as.factor(c(as.character(output$predictions_train),
                                        as.character(output$predictions_test)))
    } else {
      output$predictions <- c(output$predictions_train,
                              output$predictions_test)
    }
    output$hitrate_train <- mean(as.character(output$predictions_train)==
                                     as.character(y[subset]))
    if (!is.null(output$predictions_test) & !any(is.na(y[-subset]))) {
      output$hitrate_test <- mean(as.character(output$predictions_test)==
                                      as.character(y[-subset]))
      output$crosstable <- table(as.character(output$predictions_test),
                                 as.character(y[-subset]))
    } else {
...
model_rf <- function(x, y, subset, ...) {
    ...
    output$model <- randomForest(x[subset, ], y[subset], xtest = x_test,
                                 ytest = y[-subset], votes = TRUE,
                                 importance = TRUE)
    output$variable_importance <- importance(output$model)
    output$votes <- output$model$test$votes
    output$predictions_train <- output$model$predicted %>%
        as.character() %>%
        as.numeric()
    if (!is.null(output$model$test$predicted)) {
      output$predictions_test <- output$model$test$predicted %>%
          as.character() %>%
          as.numeric()
    } else {
...
model_nn <- function(x, y, subset, ...) {
  ...
  train <- bind_cols(data.frame(y[subset], stringsAsFactors = FALSE),
                     x[subset, ])
  train_h2o <- as.h2o(train, "train")

  # Run model
  output$model <- h2o.deeplearning(x = 2:ncol(train),  # col nrs for predictors
