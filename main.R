...
matches %<>%
  by_row(function(x) {create_summary(matches_single, x$date, x$local_team, 'local')}, .collate='cols') %>%
  by_row(function(x) {create_summary(matches_single, x$date, x$visit_team, 'visit')}, .collate='cols') %>%
...
x <- matches %>%
  filter(!is.na(result)) %>%
  select(matches('_rank'), matches('_points'), matches('_match'), matches('_change'), matches('_previous_'), matches('_avg_'), friendly)
